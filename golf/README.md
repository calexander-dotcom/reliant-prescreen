...
text to paste into the group chat: each nine's final one-down standing with
who pressed under it, the all-day bet, the greenies that were won, and where
everyone finished. Who pays whom is left to the group.
...
- Each nine starts with a **tee flip** — on the 1st tee and again on the
  10th. The side that wins it is one up in that nine's opening bet before a
  ball is hit, which by the rule above opens the first press: the tee reads
  `+1/0` from the winners' side. The 1st and 10th hole screens ask who won it
  first thing, and the Bets tab has both tees. A flip is a hole of its own
  nine's game only; the 18-hole bet starts square. On by default, with a
  **Tee flips** switch in the bet setup.
