...
import type { BetConfig, NassauConfig, Player, Round, SkinsConfig } from "@/lib/types";
      ...
      {round.bets.map((bet) =>
        bet.kind === "nassau" ? (
          <NassauFields
            ...
            onChange={replace}
            onRemove={() => setBets(round.bets.filter((b) => b.id !== bet.id))}
          />
        ) : (
          <SkinsFields
            key={bet.id}
...
function NassauFields({
  ...
  /** Which side a player is on, or "out" if they sit this bet out. */
  const sideOf = (playerId: string): "a" | "b" | "out" => {
    if (bet.sides[0].playerIds.includes(playerId)) return "a";
    if (bet.sides[1].playerIds.includes(playerId)) return "b";
    return "out";
  };

  const assign = (playerId: string, target: "a" | "b" | "out") => {
    const sides: [NassauConfig["sides"][0], NassauConfig["sides"][1]] = [
      {
        ...bet.sides[0],
        playerIds: bet.sides[0].playerIds.filter((id) => id !== playerId),
      },
      {
        ...bet.sides[1],
        playerIds: bet.sides[1].playerIds.filter((id) => id !== playerId),
      },
    ];
    if (target === "a") sides[0].playerIds = [...sides[0].playerIds, playerId];
    if (target === "b") sides[1].playerIds = [...sides[1].playerIds, playerId];
    onChange({ ...bet, sides });
  };
  ...
  return (
    <Card>
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-2">
        ...
        <Field label="Press cap" hint="Per segment. One-down presses multiply fast.">
          <select
      ...
      <div className="mt-4">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          <Field label="Side A name">
            <input
              value={bet.sides[0].name}
              onChange={(event) =>
                onChange({
                  ...bet,
                  sides: [{ ...bet.sides[0], name: event.target.value }, bet.sides[1]],
                })
              }
              className={inputClass}
            />
          ...
          <Field label="Side B name">
            <input
              value={bet.sides[1].name}
              onChange={(event) =>
                onChange({
                  ...bet,
                  sides: [bet.sides[0], { ...bet.sides[1], name: event.target.value }],
                })
              }
              className={inputClass}
            />
